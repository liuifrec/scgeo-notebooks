...
def reviewer_rows() -> pd.DataFrame:
    ...
    rows=[
        ...
        ["Reviewer 1","R1-scanorama","Scanorama dependence","Document Scanorama as one tested preprocessing/representation in a public demonstration, not a ScGeo requirement.","notebooks/data_prep/04_scgeo_gse280305_phase1_qc.ipynb; scripts/assemble_reviewer2_repairs.py","representation_metric_inventory.csv","GSE249479 and GSE211713 used no Harmony or Scanorama.","No condition-mixing optimization was performed in GSE249479/GSE211713.","Original public demonstration remains representation-specific.","Methods; Limitations","ScGeo accepts user-supplied representations; Scanorama is neither required nor used in GSE249479 and GSE211713.","complete"],
        ["Reviewer 1","R1-single-dataset","Single-dataset concern","Add public GSE249479 and replicate-aware radiation GSE211713 alongside pancreas and synthetic evidence.","notebooks/public_validation/gse249479; notebooks/public_validation/gse211713","GSE249479/GSE211713 main figures and evidence ledgers","GSE249479: 34,432 cells, descriptive_only; GSE211713: 20 mouse libraries and three replicate-aware primary contrasts.","GSE249479 lacks biological replication; GSE211713 groups remain small.","Cross-study evidence is heterogeneous and not a meta-analysis.","Results","We added two public validations with explicit design-specific inference status.","complete"],
        ["Reviewer 1","R1-validation-strength","Validation strength","Expose negative, neutral, sensitivity, exact-permutation, and comparator evidence.","notebooks/benchmarks; notebooks/public_validation/gse249479; notebooks/public_validation/gse211713","negative_neutral_or_unstable_states.csv; official Augur comparison; exact permutations","GSE211713 exact assignment counts are 126, 126, and 70; GSE249479 compares seven marker-inferred states with official R Augur.","Small-n permutation resolution and marker uncertainty remain.","No causal or population-wide generalization.","Results; Supplement","Validation now includes controlled, public, comparator, replicate-aware, and negative evidence.","complete"],
...
def global_outputs() -> None:
    ...
    methods=pd.DataFrame([
        ...
        ["GSE249479","cell/state; no biological sample unit","ScGeo geometry plus official R Augur separability","five representations; official Augur primary","descriptive_only"],
        ["GSE211713","mouse/GSM biological sample","geometric-median mouse centers, 500 mouse bootstraps, exact mouse-label permutations","PCA30/PCA50/scVI primary; PCA20/diffusion sensitivity","replicate-aware association"],
    ...
    limitations=pd.DataFrame([
        ["GSE249479 replication","No valid biological replicate identifier; all results descriptive_only.","major","biological inference","Retain explicit limitation in text and figures."],
        ["GSE249479 annotation","Seven marker-inferred states and sparse HSC-I signature.","major","state interpretation","Avoid rare-state overinterpretation."],
        ["GSE211713 design","Cross-sectional independent mice, not longitudinal.","major","time interpretation","Describe differences, not within-animal change, reversal, persistence, or causality."],
        ["GSE211713 sample size","5-vs-4 and 4-vs-4 designs yield discrete exact-permutation resolution.","major","uncertainty","Show individual mice and exact grids."],
        ["Related contrasts","Control mice are reused across GSE211713 contrasts.","major","multiplicity/dependence","Do not combine p-values or treat contrasts as independent."],
    ...
    figrows=[]
    for dataset,base in [("GSE249479",BBASE/"manuscript"),("GSE211713",CBASE/"manuscript"),("Reviewer 2",OUT/"reviewer2")]:
...
def audit_checksums() -> tuple[pd.DataFrame,bool]:
    items=[
        (Path("/home/liuyuchen/hsc_memory_nature_2026/results/gse249479_revision_qc_hvg.h5ad"),"86eed5f139e2d40ed2560d476198c11dbae423321a828d91e467013c5757b759","GSE249479 compact H5AD"),
        (Path("/home/liuyuchen/hsc_memory_nature_2026/results/gse249479_revision_representations.h5ad"),"5d91ecd63f62b14d77fab04943fb5cd0f530e795ccbc0403616036568e3d9979","GSE249479 representations"),
        (Path("/home/liuyuchen/data/gse211713/gse211713_revision_qc_hvg.h5ad"),"340504d5c88e60a3fc70f0c044ec9f65010226e99fa976e447fa8e6698418560","GSE211713 compact H5AD"),
        (Path("/home/liuyuchen/data/gse211713/gse211713_revision_annotated.h5ad"),"6f7dd2c10112960a4dca4a41b56fcc670a99e16232e0b9d86365ad5fdd66a6ca","GSE211713 annotated H5AD"),
        (Path("/home/liuyuchen/data/gse211713/gse211713_revision_representations_v2.h5ad"),"e8728846aff10091166b3f7d82b34afaf207142957b4daaafb8d6c496ca24cf2","GSE211713 representations v2"),
    ]
    ...
    for rel,expected in cval["source_hashes_before"].items():
        actual=sha256(CBASE/rel); rows.append({"artifact":"GSE211713 canonical source","path":str(CBASE/rel),"expected_sha256":expected,"actual_sha256":actual,"status":"passed" if actual==expected else "failed"})
    rows.append({"artifact":"GSE249479 manuscript pin comparison","path":str(BBASE/"manuscript/dataset_b_manuscript_assembly_validation.json"),"expected_sha256":"source_hashes_before == source_hashes_after","actual_sha256":str(bval.get("source_hashes_unchanged")),"status":"passed" if bval.get("source_hashes_unchanged") else "failed"})
...
def audit_terminology() -> tuple[pd.DataFrame,bool]:
    ...
    checks.append({"check":"GSE211713 primary inference labels","matches":int((primary.inference_status!="replicate_aware_primary").sum()),"paths":"scgeo_v2/primary_consensus_state_evidence.csv","status":"passed" if (primary.inference_status=="replicate_aware_primary").all() else "failed"})
    bfull=pd.read_csv(BBASE/"scgeo/04_full_state_evidence.csv")
    checks.append({"check":"GSE249479 remains descriptive_only","matches":int((bfull.inference_status!="descriptive_only").sum()),"paths":"GSE249479 full evidence","status":"passed" if (bfull.inference_status=="descriptive_only").all() else "failed"})
...
def validate_manuscript_and_reviewer() -> tuple[list[dict],bool]:
    ...
    checks += [{"gate":"GSE211713 hashes unchanged","expected":True,"actual":val.get("numerical_sources_unchanged"),"status":"passed" if val.get("numerical_sources_unchanged") else "failed"},{"gate":"invalidated numerical sources excluded","expected":False,"actual":val.get("invalidated_numerical_sources_used"),"status":"passed" if not val.get("invalidated_numerical_sources_used") else "failed"}]
...
def finalize() -> dict:
    ...
    unresolved=pd.DataFrame([
        ...
        ["scientific","GSE249479 has no independent biological replicate identifier.","retain descriptive_only wording"],
        ["scientific","GSE211713 is cross-sectional with small groups and reused controls.","avoid longitudinal/causal claims and p-value combination"],
