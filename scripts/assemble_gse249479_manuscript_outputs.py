#!/usr/bin/env python3
"""Assemble frozen GSE249479 evidence into manuscript artifacts.
...
def main_figure(data: dict[str, Any]) -> None:
    ...
    fig.suptitle("GSE249479: descriptive treatment geometry across conservative HSPC states", fontsize=16, fontweight="bold")
...
def write_ledgers_and_text(data: dict[str, Any], hashes: dict[str, str]) -> None:
    ...
    limitations={
        "dataset":"GSE249479","inference_status":INFERENCE_STATUS,"valid_biological_replicate_unit":False,
    ...
    main_caption="""**GSE249479, descriptive treatment geometry.** (A) PCA50-derived UMAPs display conservative marker-inferred states and condition distributions; UMAP is visualization only and is not used for quantitative geometry. (B) Primary-consensus normalized displacement for PBS→TNF and PBS→LPS, with filled symbols indicating `stable_effect` across PCA30, PCA50 and scVI. (C) Representation sensitivity across the three primary representations, PCA20 dimensional sensitivity and exploratory diffusion map. (D) Abundance change, robust displacement and within-state energy distance are shown as separate columns with independent scales; no composite score is formed. (E) Official R Augur 1.0.3 treatment separability versus ScGeo representation-stable displacement; the quantities are complementary, not equivalent. (F) GSE249479 is `descriptive_only` because no independent biological replicate identity is available. Cells, conditions, libraries and SouporCell clades are not replicates."""
